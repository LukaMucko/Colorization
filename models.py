...
def test_discriminator(patch=True):
    model = Discriminator(3,patch)
    l = torch.randn(5, 1, 256, 256)
    ab = torch.randn(5, 2, 256, 256)
    y = model(l, ab)
    print("Input:", l.size(), ab.size())
    print("Output:", y.size())
    

def init_weights(net, init="normal", std=0.02):
    for m in net.modules():
        if isinstance(m, (nn.Conv2d, nn.ConvTranspose2d)):
            if init == "normal":
                nn.init.normal_(m.weight.data, 0.0, std)
            elif init == "xavier":
                nn.init.xavier_normal_(m.weight.data, gain=std)
            elif init == "kaiming":
